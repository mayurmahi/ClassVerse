...
/* ─────────────────────────────────────────────────────────────────────────────
   Helpers
───────────────────────────────────────────────────────────────────────────── */
const getDownloadUrl = (url, format) => {
  if (!url) return "#";
...
// ── FIX: View URL helper ───────────────────────────────────────────────────────
// Problem: Cloudinary serves raw files (pdf, pptx, docx…) with
//   Content-Disposition: attachment  →  browser always downloads.
// Fix:
//   • PDF / images  → insert fl_inline into the Cloudinary URL so the browser
//                     opens the file directly instead of downloading it.
//   • PPTX / DOCX   → wrap with Google Docs Viewer which can render Office
//                     files right in a browser tab.
const getViewerUrl = (url, fileType) => {
  if (!url) return "#";
  const t = (fileType || "").toLowerCase();

  // Office formats — Google Docs Viewer renders these in the browser
  const officeTypes = ["pdf","ppt", "pptx", "doc", "docx"];
  if (officeTypes.includes(t)) {
    return `https://docs.google.com/viewer?url=${encodeURIComponent(url)}&embedded=true`;
  }

  // PDF and images — fl_inline tells Cloudinary to send Content-Disposition: inline
  if (url.includes("res.cloudinary.com")) {
    return url.replace("/upload/", "/upload/fl_inline/");
  }

  // Fallback for any non-Cloudinary URL (e.g. legacy localhost)
  return url;
};
            ...
            <div key={m._id} className="bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
              ...
              <div className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {/* ── FIX: View button — opens file in browser instead of downloading ── */}
                  <a
                    href={getViewerUrl(m.fileUrl, m.fileType)}
                    target="_blank"
                    rel="noreferrer"
