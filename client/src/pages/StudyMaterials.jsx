...
const QuizCreatorModal = ({ material, classroomId, onClose, onCreated }) => {
  ...
  // Load AI suggestions automatically when modal opens
  ...
  const updateSuggestedMarks = (id, val) =>
    setSuggestedQs((prev) => prev.map((q) => q.id === id ? { ...q, suggestedMarks: Number(val) } : q));
  ...
  const updateCustom = (id, field, val) =>
    setCustomQs((prev) => prev.map((q) => q.id === id ? { ...q, [field]: val } : q));

  const updateOption = (id, idx, val) =>
    setCustomQs((prev) => prev.map((q) => {
      if (q.id !== id) return q;
      const opts = [...q.options];
      const oldVal = opts[idx];
      opts[idx] = val;
      // Clear answer if it matched the old option text
      const answer = q.answer === oldVal ? "" : q.answer;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-3">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[93vh] flex flex-col overflow-hidden">
        ...
        <div className="flex border-b border-gray-100 bg-gray-50/80 flex-shrink-0">
          {[
            { key: "suggest", label: "1. AI Suggestions" },
            { key: "build", label: "2. Build Quiz" },
            { key: "details", label: "3. Publish" },
          ].map((s) => (
            <button key={s.key}
              onClick={() => {
                if (s.key === "details") { const err = validateBuild(); if (err) { alert(err); return; } }
                setStep(s.key);
              }}
              className={`flex-1 text-xs font-semibold py-3 border-b-2 transition-colors ${
                step === s.key
                  ? "border-[#2E75B6] text-[#1F4E79] bg-white"
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Tick questions you want to include. You can adjust marks per question.</p>
                </div>
                <button onClick={fetchSuggestions} disabled={suggestLoading}
                  ...
                  <svg className={`w-3.5 h-3.5 ${suggestLoading ? "animate-spin" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              ...
              {suggestLoading ? (
                <div className="flex flex-col items-center py-16 gap-3">
                  <svg className="w-8 h-8 text-[#2E75B6] animate-spin" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
                  </svg>
                ...
                <div className="space-y-3">
                  {suggestedQs.length === 0 && !suggestError && (
                    <p className="text-center text-sm text-gray-400 py-8 border-2 border-dashed border-gray-200 rounded-xl">
                      No suggestions loaded. Click "Regenerate" or proceed to add questions manually.
                    </p>
                  )}
                  {suggestedQs.map((q) => (
                    <div key={q.id} onClick={() => toggleSelect(q.id)}
                      className={`border rounded-xl p-4 cursor-pointer transition-all select-none ${
                        selectedIds.has(q.id) ? "border-[#2E75B6] bg-blue-50/40" : "border-gray-200 hover:border-gray-300 bg-white"
                      }`}>
                      <div className="flex items-start gap-3">
                        {/* Checkbox */}
                        <div className={`w-5 h-5 rounded flex-shrink-0 mt-0.5 border-2 flex items-center justify-center transition-colors ${
                          selectedIds.has(q.id) ? "bg-[#2E75B6] border-[#2E75B6]" : "border-gray-300 bg-white"
                        }`}>
                          ...
                          {selectedIds.has(q.id) && (
                            <div className="flex items-center gap-2 mt-2" onClick={(e) => e.stopPropagation()}>
                              ...
                              <input type="number" min="1" max="10"
                                value={q.suggestedMarks || 1}
                                onChange={(e) => updateSuggestedMarks(q.id, e.target.value)}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                ...
                <button onClick={() => setStep("build")}
                  className="bg-[#1F4E79] hover:bg-[#2E75B6] text-white text-sm font-semibold px-5 py-2 rounded-xl transition-colors">
                  Next: Build Quiz →
                </button>
              </div>
            </>
          ...
          {step === "build" && (
            <>
              {/* Show selected AI questions (read-only summary) */}
              {selectedIds.size > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                    From AI ({selectedIds.size} selected)
                  </p>
                      ...
                      <div key={q.id} className="bg-blue-50/50 border border-blue-100 rounded-xl p-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-xs bg-blue-100 text-blue-700 font-bold px-2 py-0.5 rounded-full flex-shrink-0">
                            {q.suggestedMarks || 1} mk
                          </span>
                        </div>
                        <div className="grid grid-cols-2 gap-1 mt-2">
                          {q.options.map((opt, oi) => (
                            <div key={oi} className={`text-xs px-2 py-1 rounded-lg border ${
                              opt === q.answer ? "bg-emerald-50 border-emerald-300 text-emerald-700 font-semibold" : "bg-white border-gray-200 text-gray-500"
                            }`}>
                              {opt === q.answer ? "✓ " : ""}{opt}
                            </div>
                          ))}
              ...
              {customQs.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                    Your Custom Questions ({customQs.length})
                  </p>
                      ...
                      <div key={q.id} className="border border-gray-200 rounded-xl p-4 bg-white space-y-3">
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                            Question {selectedIds.size + idx + 1}
                          </p>
                          <button onClick={() => removeCustom(q.id)}
                        ...
                        <textarea placeholder="Enter question text…" value={q.question}
                          onChange={(e) => updateCustom(q.id, "question", e.target.value)} rows={2}
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                          <div className="flex-1 min-w-40">
                            ...
                            <select value={q.answer}
                              onChange={(e) => updateCustom(q.id, "answer", e.target.value)}
                          ...
                          <div>
                            ...
                            <input type="number" min="1" max="20" value={q.marks}
                              onChange={(e) => updateCustom(q.id, "marks", Number(e.target.value))}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setStep("suggest")}
                    className="text-sm text-gray-500 hover:text-gray-700 font-semibold px-4 py-2 rounded-xl transition-colors">
                    ← Back
                  </button>
            ...
            <>
              ...
              <div>
                ...
                <input type="text" value={quizTitle} onChange={(e) => setQuizTitle(e.target.value)}
                  ...
                  placeholder="e.g. Chapter 3 Mid-Term Quiz"
                />
              </div>
              <div>
                ...
                <textarea value={quizDesc} onChange={(e) => setQuizDesc(e.target.value)} rows={3}
                  ...
                  placeholder="Any instructions or notes for students…"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Deadline *</label>
                <input type="datetime-local" min={minDeadline} value={deadline}
...
───────────────────────────────────────────────────────────────────────────── */
const SubmissionsModal = ({ quiz, onClose }) => {
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            <div className="flex items-center justify-center py-16">
              <svg className="w-7 h-7 text-[#2E75B6] animate-spin" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
              </svg>
            </div>
              ...
              {submissions.map((s) => (
                <div key={s._id} className="border border-gray-100 rounded-xl p-4 flex items-center justify-between gap-4">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-xl font-black text-[#1F4E79]">{s.totalMarksAwarded}<span className="text-gray-400 font-normal text-sm">/{s.totalMarks}</span></p>
...
const StudentQuizModal = ({ quiz, onClose, onSubmitted }) => {
  ...
  const selectAnswer = (qi, opt) => {
    if (result || expired || quiz.submission) return;
  ...
  // Questions to render (after submit, use result.questions which have answers)
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-3">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* Result panel after fresh submission */}
          {result && (
...
/* ─────────────────────────────────────────────────────────────────────────────
   Classroom-wide Quiz Board (used ONLY by Students from the Quizzes tab)
   This is a separate standalone component rendered outside materials list.
───────────────────────────────────────────────────────────────────────────── */
export const StudentQuizBoard = ({ classroomId }) => {
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-16">
      <svg className="w-7 h-7 text-purple-500 animate-spin" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
      </svg>
    </div>
  );
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${
              ...
              {q.submission ? "Submitted" : q.isExpired ? "Expired" : "Active"}
            </span>
            {q.materialId?.title && (
              <span className="text-[11px] text-gray-400 truncate max-w-[150px]">📄 {q.materialId.title}</span>
            )}
          </div>
        ...
        <div className="flex-shrink-0">
          ...
          {q.submission && (
            <button onClick={() => setActiveQuiz(q)}
              className="border border-purple-200 text-purple-600 hover:bg-purple-50 text-xs font-semibold px-4 py-2 rounded-xl transition-colors">
              View Result
            </button>
          )}
          {q.isExpired && !q.submission && (
            <span className="text-xs text-gray-400 px-3 py-2">Expired</span>
          )}
        </div>
      </div>
    </div>
...
/* ─────────────────────────────────────────────────────────────────────────────
   Per-material quiz panel (inside each material card, shown for both roles)
───────────────────────────────────────────────────────────────────────────── */
const MaterialQuizPanel = ({ material, classroomId, userRole, allQuizzes, onRefresh, onCreateQuiz }) => {
  ...
  // Filter to quizzes belonging to this material
  const matQuizzes = allQuizzes.filter(
    (q) => q.materialId?._id === material._id || q.materialId === material._id
  );
    ...
    <div className={`border-t px-5 py-3 ${hasActive && userRole === "Student" ? "border-purple-100 bg-purple-50/20" : "border-gray-50 bg-gray-50/30"}`}>
      <button onClick={() => setExpanded((v) => !v)}
        className={`flex items-center gap-2 text-xs font-semibold transition-colors ${
          hasActive && userRole === "Student" ? "text-purple-600 hover:text-purple-800" : "text-indigo-500 hover:text-indigo-700"
        }`}>
        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3M12 17h.01"/>
        </svg>
        {hasActive && userRole === "Student" ? `🔔 Quiz Available (${matQuizzes.filter((q) => !q.isExpired && !q.submission).length})` : `Quizzes${matQuizzes.length > 0 ? ` (${matQuizzes.length})` : ""}`}
        ...
        <div className="mt-3 space-y-2">
          {matQuizzes.length === 0 ? (
            <p className="text-xs text-gray-400 py-1">
              {userRole === "Teacher" ? "No quizzes yet for this material." : "No quizzes for this material."}
            </p>
          ) : (
            matQuizzes.map((q) => (
              <div key={q._id} className="bg-white border border-indigo-100 rounded-xl p-3 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${
                      q.submission ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {userRole === "Student" && !q.submission && !q.isExpired && (
                    <button onClick={() => setActiveStudentQuiz(q)}
                      className="text-xs font-semibold text-white bg-purple-600 hover:bg-purple-700 px-3 py-1.5 rounded-lg transition-colors">
                      Take Quiz
                    </button>
                  )}
                  {userRole === "Student" && q.submission && (
                    <button onClick={() => setActiveStudentQuiz(q)}
                      className="text-xs font-semibold text-purple-600 border border-purple-200 hover:border-purple-400 px-3 py-1.5 rounded-lg transition-all">
                      View Result
                    </button>
                  )}
                </div>
      ...
      {activeSubmissionsQuiz && (
        <SubmissionsModal
          quiz={activeSubmissionsQuiz}
          onClose={() => setActiveSubmissionsQuiz(null)}
        />
      )}
    </div>
  );
...
const StudyMaterials = ({ classroomId }) => {
  ...
  const [quizzes, setQuizzes] = useState([]);           // ← all quizzes loaded once
  ...
  // KEY FIX: load ALL quizzes for the classroom once on mount
  // so students see quizzes without needing to expand anything
  ...
  // How many active quizzes are pending for the student
  const pendingQuizCount = quizzes.filter((q) => !q.isExpired && !q.submission).length;

  return (
    <div className="space-y-5">

      {/* ── Student: pending quiz alert banner ── */}
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-2xl px-5 py-3.5 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-purple-800">
              {pendingQuizCount} quiz{pendingQuizCount !== 1 ? "zes" : ""} waiting for you
            </p>
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            {uniqueTypes.map((t) => (
              <button key={t} onClick={() => setFilterType(t)}
                className={`text-xs font-semibold px-3 py-2 rounded-xl border uppercase transition-colors ${filterType === t ? "bg-[#1F4E79] text-white border-[#1F4E79]" : "bg-white text-gray-500 border-gray-200 hover:border-[#2E75B6]"}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {filtered.map((m) => (
            <div key={m._id} className="bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
              ...
              <div className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className={`text-[11px] font-bold uppercase px-2 py-0.5 rounded-md ${fileTypeBadge(m.fileType)}`}>
                      {m.fileType}
                    </span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <a href={"http://localhost:5000/uploads/" + m.filePath} download
              ...
              {/* Quiz panel — both roles, uses pre-loaded quizzes */}
              <MaterialQuizPanel
                material={m}
                ...
                onCreateQuiz={(mat) => setQuizCreatorMaterial(mat)}
              />
            </div>
          ))}
        </div>
